...
`shared/schema.ts` defines the Drizzle table shapes (`contactSubmissions`, `newsletterSubscriptions`, `seoPages`, `blogPosts`) used both for type inference in `shared/routes.ts` and as the live tables `DatabaseStorage` queries.
...
Single-admin auth (`server/auth.ts`): a signed httpOnly cookie (`gad_admin_session`, stdlib `crypto` HMAC — no session table, deliberately stateless for Vercel serverless). Credentials live in env vars: `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH` (scrypt hash, generate with `npm run admin:hash-password -- <password>`), `SESSION_SECRET`. `requireAdminAuth` middleware guards every `/api/admin/*` route except `/api/admin/login` and `/api/admin/logout` (logout only clears the cookie, so no auth check is needed).
...
`passport`/`express-session`/`connect-pg-simple` are unused leftover dependencies — the real auth is the custom cookie scheme in `server/auth.ts`; all non-`/api/admin/*` endpoints are public.
